                ...
                success: function (data) {
                  ...
                  if (data.state) {
                    ...
                    if (data.type === "not_allowed") {
                      errorDialog.html(
                        "اسم النطاق قد تم حجزه مسبقا من طرف احد العملاء لمرجو ادخال اسم اخر"
                      );
                      errorDialog.show();
                    }
                  }
...
function prepareContentToDownload(_this) {
  ...
  _this.pages.getPagesObjInArray().forEach(function (pObj, indx) {
    ...
    if (sections) {
      Array.prototype.forEach.call(sections, function (li) {
        ...
        if (
          ...
          pObj.sections[li.dataset.group][cloneSection.id].modal.forEach(
            function (modalId) {
        ...
        var sectionJs =
          script.innerHTML.replace(
            /\/\/delete[\s\S]*?\/\/deleteend|\n?\/\/magnific[\s\S]*?\/\/magnificend/gim,
            ""
          ) || "";

        contentJs += sectionJs;

        var cookieDependent = sectionJs.match(
          /\/\/cookie-dependent-start([\s\S]*?)\/\/cookie-dependent-end/im
        );
        cookieDependentJs += cookieDependent ? cookieDependent[1] : "";
        var windowsLoad = sectionJs.match(
          /\/\/windows-load-start([\s\S]*?)\/\/windows-load-end/im
        );
        windowsLoadJs += windowsLoad ? windowsLoad[1] : "";
        contentJs =
          contentJs.replace(
            /\/\/cookie-dependent-start[\s\S]*?\/\/cookie-dependent-end|\/\/windows-load-start[\s\S]*?\/\/windows-load-end/gim,
            ""
          ) || "";
      });
    }
    ...
    data.pages.push({
      ...
      preloader:
        pObj.preloader && pObj.preloader.html
          ? pObj.preloader.html.replace(/\?t=[0-9]*/gi, "")
          : "",
      js: js,
      style: style,
    });
