...
class Core {
  ...
  isPaused = template ? true : this.checkEditPrjID;
  ...
  translateWord = {
    keywords_ar: {
      ...
      rightPanel: {
        ...
        addItemBefore:
          "<bdo dir='rtl'>أضف أي عنصر على اللوحة لتفعيل قائمة الإعدادات.</bdo>",
      ...
      popup: {
        form: {
          ...
          getText: {
            ...
            paragraphAdded:
              '<bdo dir="rtl" class="">سيتم إضافة الفقرة، يُمكنك تعديل محتواها في منشئ الصفحات.<br>المرجو النقر مرتين على حقل نص التعليق في منشئ الصفحات!</bdo>',
          },
        },
        button: {
          apply: "<bdo dir='rtl'>تطبيق</bdo>",
        ...
        cantAddItem:
          "<bdo dir='rtl'>عذراً، لا يسمح بإضافة قسمين من هذا العنصر!</bdo>",
        cancel: "<bdo dir='rtl'>إلغاء</bdo>",
      },
    },
  };
  ...
  addWrapDrag(pageHTML) {
    ...
    const lang = this.translateWord.keywords_ar;
    const iframSvgAnimation = this.iconSvg.iframSvgAnimation;
  ...
  setEventListenersToModals() {
    ...
    const modalShowHide = function (e) {
      const item = this.popupThumb.querySelector(
  ...
  _showPopupThumb(item) {
    ...
    if (item.dataset.toggle === "modal" && item.classList.contains("active")) {
      this.windowIframe.jQuery(item.dataset.target).modal("hide");
      item.classList.remove("active");
    } else if (
      ...
      const modalShow = function (el) {
        const $modal = this.windowIframe.jQuery(el.dataset.target);
        const li = $modal[0].parentElement;

        $modal.modal("show");
        this.activeFormModal = $modal[0];
      ...
      const active = this.popupThumb.querySelector(
        ".item.active[data-toggle=modal]"
      );
      if (active) {
        const $activeModal = this.windowIframe.jQuery(active.dataset.target);
      ...
      const $alert = this.windowIframe.jQuery(item.dataset.target);
      ...
      const modalAlert = this.main.querySelector(item.dataset.target);
      ...
      const $panel = this.windowIframe.jQuery(item.dataset.target);
      ...
      const modalPanel = this.main.querySelector(item.dataset.target);
  ...
  reloadScript(container) {
    ...
    Array.prototype.forEach.call(smooth, function (a) {
      this.windowIframe.jQuery(a).smoothScroll({
        speed: 800,
  ...
  StylingWrapParent(li) {
    ...
    var elements = li.querySelectorAll(
      ".buttons-control, .buttons-control-form"
    );
    Array.prototype.forEach.call(elements, function (el) {
      ...
      if (parent) {
        ...
        el.addEventListener(
          "mouseenter",
          this.EventMoseEnterOnChildActive.bind(parent)
        );
      }
    });
  }
  RebuildControl(wrap) {
    ...
    if (wrap.classList.contains("spr-oc-show")) {
      ...
      var li = this.findParent(wrap, ["section-item"]);
    ...
    Array.prototype.forEach.call(wrapControl, function (element) {
      ...
      if (element.classList.contains("wrap-control")) {
        ...
        var section = this.findParent(parent, ["section-item", "modal-dialog"]);
      }
  ...
  ConvertAllImagesToSvg(context) {
    ...
    [].forEach.call(images, function (img) {
      this.ConvertImgToSvg(img.getAttribute("src"), img);
    });
  }
  ...
  RemoveNowrapSuperStructure(li, typeElements, mode) {
    ...
    if (this.drGAndDropEl) {
      this.drGAndDropEl.destroy();
      this.drGAndDropEl = null;
    }
    ...
    this.documentIframe.removeEventListener(
      "click",
      this._clickOnDocumentIframeEditElementMode
    );

    if (this._arrEditorText.length > 0) {
      this._arrEditorText.forEach(function (editor) {
  ...
  ClearGalleryOnPage(page) {
    ...
    Array.prototype.forEach.call(galleries, function (gallery) {
      this.RemoveOwlSuperstructure(gallery);
    });
  }
  ...
  RefreshParallax(target) {
    ...
    if (this.windowIframe.rellax) {
      this.windowIframe.rellax.destroy();
    }

    if (target.querySelector(".parallax")) {
      this.windowIframe.rellax = new this.windowIframe.Rellax(".parallax", {
        center: true,
      });
    }
  }
  ...
  cloneObject(Obj) {
    ...
    if (Obj instanceof Array) {
      ...
      Obj.forEach(function (el, item) {
        clone[item] = this._checkElmtsForCloneObj(this, el);
      });
    } else {
      var clone = {};
      for (var item in Obj) {
        clone[item] = this._checkElmtsForCloneObj(this, Obj[item]);
      }
    }
  ...
  _checkElmtsForCloneObj(_this, Obj) {
    console.log("_this _this this", _this);
    var result = null;
    if (
  ...
  addNewForm(form, section, oldSection, copyElement) {
    ...
    this.forms[sectionId] = { sendMode: "simple-mail-php" };
  ...
  applyMagnificPopup(DOMElement) {
    ...
    if (
      ...
      this.windowIframe.jQuery(DOMElement).magnificPopup({
        type: "iframe",
        key: "video-key",
        iframe: {
          patterns: {
      ...
      var li = this.findParent(DOMElement, ["section-item", "modal-confirm"]);
  ...
  applyMagnificPopupGallery(gallery) {
    ...
    this.windowIframe.jQuery(gallery).magnificPopup({
      ...
      callbacks: {
        open: function () {
          this.windowIframe.jQuery(gallery).trigger("stop.owl.autoplay");
        },
        close: function () {
          this.windowIframe.jQuery(gallery).trigger("play.owl.autoplay");
        },
      },
    });
  }
  _reloadMagnific(page) {
    ...
    if (magnificGallery) {
      Array.prototype.forEach.call(magnificGallery, function (gallery) {
        this.applyMagnificPopupGallery(gallery);
      });
    }
  ...
  _reloadVideoBg(page, mode) {
    ...
    if (vides) {
      Array.prototype.forEach.call(vides, function (vide, indx) {
        if (mode === "run") {
          if (this.windowIframe.jQuery(vide).data("vide")) {
            this.windowIframe.jQuery(vide).data("vide").destroy();
          }
          vide.innerHTML = "";
          this.windowIframe.jQuery(vide).vide();
        } else if (mode === "reload") {
  ...
  _prepareContentPagesToSave(mode) {
    ...
    var main = this.main.cloneNode(true);
    this.clearControlElements(main);
    this._DeleteTransformFromAttribute(main);

    var sectionsName = JSON.stringify(Object.keys(this.projectSections));
    var pagesStorageObj = JSON.stringify(this.pages.getPagesObjInArray());
    var projectData = JSON.stringify(main.innerHTML);
    var modalContainer = JSON.stringify(this.modalContainer.innerHTML);
    var modalFormContainer = JSON.stringify(this.modalFormContainer.innerHTML);
    var forms = JSON.stringify(this.forms);
    var video_bg = JSON.stringify(this._videoBg);
    var gallery = JSON.stringify(this._owlGallery);
    var form_section = JSON.stringify(this._formSection);
    var smooth = JSON.stringify(this._smooth);
    var parallax = JSON.stringify(this._parallax);
    var datepicker = JSON.stringify(this._datepicker);
    var filefield = JSON.stringify(this._filefield);
    var countup = JSON.stringify(this._countup);
    var countdown = JSON.stringify(this._countdown);
    var gMaps = JSON.stringify(this.gMaps);
    var plugins = JSON.stringify(this.plugins);
    var builderStyle = this.builderStyle.innerHTML;
    var gs = {};
    for (var item in this.globalStyle.optionsStyle) {
      if (item === "defaultPropertiesForTags") {
        gs[item] = this.globalStyle.optionsStyle[item].map(function (val) {
          var op = {};
        ...
        gs[item] = this.globalStyle.optionsStyle[item];
      }
    }
  ...
  _loadProject(data, mode) {
    ...
    if (data.error) {
      ...
      setTimeout(function () {
        this.body.classList.remove("first-show");
      }, 1600);
    ...
    this._videoBg = JSON.parse(data.video_bg);
    this._owlGallery = JSON.parse(data.gallery);
    this._formSection = JSON.parse(data.form_section);
    this._smooth = JSON.parse(data.smooth);
    this._parallax = JSON.parse(data.parallax);
    this._datepicker = JSON.parse(data.datepicker);
    this._filefield = JSON.parse(data.filefield);
    this._countup = JSON.parse(data.countup);
    this._countdown = JSON.parse(data.countdown);

    this.triggerImport = true;

    var modeEdit = "edit-sections";

    if (mode === "import") {
      var datas = this._prepareContentPagesToSave("no-storage");
    ...
    if (typeof projectData === "string") {
      ...
      this.main.innerHTML = projectData;
    } else {
      this.main.appendChild(projectData);
    }
    ...
    this.globalStyle.load(optionsStyle);
    this.builderStyle.innerHTML = data.builderStyle;

    this.leftPanel = new LeftPanel();

    var sections = this.main.querySelectorAll("ul.blr-active-page > li");
    var timeLoading = sections.length * 2;
    var loadPreloader = document.querySelector(".progress .load");
    var preloaderLogo = document.querySelector(
      ".supra-preloader .logo-preloader:before"
    );
    ...
    this.clearGalleryOnPage(this.main);
    this.clearInstafeed(this.main);

    var ul = this.leftPanel.cPanels["project-pages"]
      .getDOMSelf()
      .querySelector("ul");
    if (ul !== null) {
    ...
    var pages = this.main.parentElement.querySelectorAll(".main > ul");
    var activePage = this.main.parentElement.querySelector(
    ...
    var offcanvas = this.main.querySelectorAll("nav ~ .show");
    ...
    this._style.html = this.main.children[0];
    ...
    Array.prototype.forEach.call(pages, function (page) {
      ...
      var pageObj = new Page({
        ...
        id: this.pages.getPagesObjInArray().length,
        mode: "load",
        pageHTML: pageHTML,
    ...
    this.modalContainer.innerHTML = modalContainer;
    this.modalFormContainer.innerHTML = modalFormContainer;
    this.forms = forms;
    this.gMaps = gMaps;
    this.plugins = plugins;

    this.reloadScript(activePage);
    this._reloadVideoBg(activePage, "run");

    this._setControlsElement(activePage.dataset.name, "edit-sections");
    this._changePageMode(activePage, modeEdit);
    this.leftPanel.cPanels["sections"].listenerSectionsMouseDown(activePage);
    this._idActivePage = activePage.dataset.id;

    this.leftPanel.cPanels["project-pages"]._addProjectPagesToRightSide();

    this.popupThumb.querySelector(".container-thumb").innerHTML = "";
    this.popupThumb.classList.remove("collapse-thumb");
    this.popupThumbArray = [];
    this._initThumbnails();
    this.removeNowrapSuperStructure(activePage);
    this._clearAnimationByClass(activePage);
    setTimeout(function () {
      this._refreshParallax(activePage);
    }, 1000);

    document.title = this.getActivePageObject().getPageTitle();
    document.querySelector("link[rel=icon]").href =
      this.getActivePageObject().getPageFavicon();

    this.triggerImport = false;
  }
  ...
  clearControlElements(page) {
    ...
    if (this._arrListenersEditElementModal.length > 0 && !page) {
      this._arrListenersEditElementModal.forEach(function (listenter) {
        listenter.el.removeEventListener("mouseover", listenter.func);
      });
    } else {
      if (this._arrListenersEditElement.length > 0) {
        this._arrListenersEditElement.forEach(function (listenter) {
          listenter.el.removeEventListener("mouseover", listenter.func);
        });
      }

      if (this._wrapEvetActiveClickEditElement.length > 0) {
        var wEACEE = this._wrapEvetActiveClickEditElement;
        wEACEE.forEach(function (wrap) {
          wrap.el.removeEventListener("click", wrap.func);
        });
        this._wrapEvetActiveClickEditElement = [];
      }

      this.clearControlButtons(page);
      ...
      if (maps) {
        Array.prototype.forEach.call(maps, function (map) {
          var li = this.findParent(map, ["section-item"]);
          map.innerHTML = "";
  ...
  _callEdit(URL, nameFile) {
    ...
    request.onload = function () {
      if (!this._triggerImport) {
        this._triggerImport = true;
        ...
        this.ajax(
          ...
          function (data) {
            ...
            var popup = this.main.querySelector(".modal.show");
            this.windowIframe.jQuery(popup).modal("hide");
            this.popupThumbArray = [];
            this.popupThumb.querySelector(".container-thumb").innerHTML = "";
            ...
            try {
              ...
              this.leftPanel.cPanels["project-pages"].createNewProject();
              this.removePreloader();
              setTimeout(function () {
                this.body.classList.remove("first-show");
              }, 1600);
  ...
  _blockingDivHidder(disabledElm, globalStyleDiv, clickedFrom) {
    let uls = this.main.querySelectorAll("ul");
  ...
  _checkTheIframDom() {
    ...
    let uls = this.main.querySelectorAll("ul");
    let blockingDiv = document.querySelector(
      "#sidebar_contentHeader-right .global-style__container .before__getting-element"
    );
    let globalStyle = document.querySelector(
