...
let gsInput = form.querySelector(
  ".form-group.interface-field-group input[data-csrf][name=interface]"
);
// GOOGLE SHEETS ID
if (gsInput) {
  ...
  form.addEventListener("submit", (e) => {
    ...
    const formValidation = () => {
      ...
      let name = form.querySelector(
        ".text-field-group input[type='text']:not('.datepicker-input')"
      );
      ...
      if (email) {
        if (
          !/^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3}(\s)*)+$/g.test(email.value)
        ) {
          let span = document.createElement("span");
      ...
      if (textarea) {
        if (!/^\S.*(?:\r?\n\s.*)*$/gmu.test(textarea.value)) {
          let span = document.createElement("span");
          span.innerHTML =
            "please describe your message in more than 50 character";
