...
class Request
{
            ...
            if ($this->_strposa(strtolower($file_name), $arr)) {
                if ($this->_validation($_FILES['data'], 'file')) {
                    if (move_uploaded_file(
                        ...
                        if ($mode === 'addgallery') {
                            echo json_encode(array('fileName' => $this->_base_url . '/images/gallery/' . $sub_folder . $file_name));
                        } else if ($mode === 'addgalleryvideo') {
                            echo json_encode(array('fileName' => $this->_base_url . '/video/gallery/' . $file_name));
                        } else if ($mode === 'import') {
                            $zip = new ZipArchive;
                            $file_name = 'public/tmp/' . $file_name;
    ...
    public function FtpUpload()
    {
        ...
        if ($this->_strposa($_POST['type'], ['ftp', 'ftps'])) {
            ...
            $output_dir = 'public/tmp/' . preg_replace('#\.zip#', '', $file_name);

            try {
                if ($zip->open('public/tmp/' . $file_name)) {
                    $zip->extractTo($output_dir);
                    $zip->close();
                    unlink('public/tmp/' . $file_name);
                }

                $ftp = new ftpUploading(
                    'public/tmp/' . preg_replace('#\.zip#', '', $file_name),
                    [
                        'type' => $_POST['type'],
                        'mode' => $_POST['mode'],
                        'host' => $_POST['host'],
            ...
            $id = time();
            $_tmp = "/" . $id;
            $output_dir = $this->_tmp_path . $_POST['dir'];
            $output_dir .= $_tmp;
   ...
   ';
    ...
    public function Getgalleryvideo()
    {
        ...
        foreach ($this->_videos as $video) {
            if (
                ...
                $response[] = array(
                    'name' => $video, 'width' => $size ? $size[0] : '', 'height' => $size ? $size[1] : ''
                );
            } else if ($this->_strposa(strtolower($video), array('.' . $data))) {
                $size = getimagesize($base_path . '/' . $video);
                $response[] = array(
                    'name' => $video, 'width' => $size[0], 'height' => $size[1]
                );
            }
        }
    ...
    public function Export()
    {
        ...
        $filename = "public/tmp/" . uniqid() . "_project.zip";
        $zip = new ZipArchive;
    ...
    {
        $this->_upload_file('public/tmp/', array('.zip', '.supra'), 'import');
    }
    ...
    protected function _add_page(
        ...
        foreach ($data->pages as $page) {
            foreach ($page->sections as $group_name => $sections) {
                if (
                    file_exists('sections/' . $group_name . '/overall.js')
           ...
           <link rel=\"stylesheet\" href=\"css/landino.css\" /> <link rel=\"stylesheet\" href=\"css/custom.css\" />" . '' . "$includePajeStyle" . '' . "$preloader_css
    ...
    private function saveSiteToTmp($dataPost)
    {
        ...
        $filename = "public/tmp/" . uniqid() . "_website.zip";

        $zip = new ZipArchive;
        $zip->open($filename, ZipArchive::CREATE);

        // array_push($baseFiles['css'], 'landino.css');
        ...
        $zip->addFromString('css/landino.css', $custom_style);
